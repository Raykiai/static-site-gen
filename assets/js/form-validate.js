	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#grubhub-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#grubhub-contact').find('input,textarea').attr({value:''});
				jQuery('#grubhub-contact').css({opacity:1});
				jQuery('#grubhub-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#grubhub-contact').animate({opacity:0.3});
		jQuery('#grubhub-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#grubhub-contact').find('input,textarea').css('border','none');
